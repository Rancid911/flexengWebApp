...
function getStatusTone(status: string) {
  if (status === "overdue") return "bg-[#fff4df] text-[#bc7500]";
            ...
            <CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-3 text-sm text-slate-500">
                  ...
                  <span>
                    {item.requiredCount > 0 ? `${item.completedRequiredCount} из ${item.requiredCount} обязательных выполнено` : "Без обязательных пунктов"}
                  </span>
                  <span>{formatDueDate(item.dueAt)}</span>
                </div>
                <p className="text-sm font-semibold text-indigo-600">Открыть задание и продолжить</p>
              </div>
              <span className={`inline-flex min-h-10 items-center rounded-full px-4 py-2 text-sm font-bold ${getStatusTone(item.status)}`}>
