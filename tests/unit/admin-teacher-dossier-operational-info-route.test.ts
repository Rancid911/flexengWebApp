...
const validPayload = {
  ...
  cooperation_type: "freelance",
  lesson_rate_amount: 2500,
  currency: "RUB"
};

function createRequest(body: Record<string, unknown>) {
  return new NextRequest("http://localhost/api/admin/teachers/teacher-1/dossier/operational-info", {
    method: "PATCH",
...
describe("PATCH /api/admin/teachers/[teacherId]/dossier/operational-info", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("upserts operational info for valid staff payload and returns DTO", async () => {
    const mocks = createSupabaseMock();
    createAdminClientMock.mockReturnValue(mocks.supabase);

    const { PATCH } = await import("@/app/api/admin/teachers/[teacherId]/dossier/operational-info/route");
  ...
  it("returns validation error for invalid operational values", async () => {
    const mocks = createSupabaseMock();
    createAdminClientMock.mockReturnValue(mocks.supabase);
    ...
    const response = await PATCH(
      createRequest({
        ...
        status: "paused",
        start_date: "2026/04/01",
        cooperation_type: "contractor",
        lesson_rate_amount: -1,
        currency: "USD"
      }),
    ...
    expect(payload.details.fieldErrors).toMatchObject({
      ...
      start_date: expect.any(Array),
      cooperation_type: expect.any(Array),
      lesson_rate_amount: expect.any(Array),
