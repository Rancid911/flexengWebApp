...
| --- | --- | --- | --- | --- |
| Practice attempts, answers, mistakes | Verified safe | Creates `student_test_attempts`, `student_test_answers`, and `student_mistakes`; may sync homework progress. | `/api/practice/attempts` calls `requireRealStudentWriteContext(...)` and `requirePermission(..., "practice.attempts.submit")` before parsing; `practice-attempts.service.ts` calls `getCurrentRealStudentWriteContext(...)` before activity loading or infrastructure creation. Focused service tests cover teacher-preview denial before repository or homework access. | No |
