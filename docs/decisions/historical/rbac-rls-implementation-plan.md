...
> Historical document. This is not current runtime truth. Use `docs/access-control-current-state.md` and `docs/README.md` for current source-of-truth documentation.
...
This is a historical roadmap and decision log, not the current runtime status source. For the current access-control model and authoritative inventory links, start with `docs/access-control-current-state.md`.
...
## Permission Vocabulary Registry

The TypeScript permission vocabulary is classified in `lib/permissions/registry.ts`.

- `canonicalPermissions` are DB-backed keys seeded in `public.permissions`.
- `legacyPermissionAliases` are compatibility keys that map to a canonical DB-backed permission.
- `runtimeCompatibilityPermissions` are app/runtime workflow keys that are not seeded directly but are backed by canonical DB permissions or dedicated RBAC-aware `can()` handling.
- `runtimeOnlyPermissions` are app-only workflow keys that are intentionally not seeded or DB-backed yet.
- `deprecatedPermissions` must stay explicit if a key is retained only for removal compatibility.

New DB-backed permissions must be added to the seed migration path and registry together. Legacy aliases must not be removed without a focused compatibility PR, and runtime-only keys should be mapped, classified as runtime compatibility, or seeded only in a domain-specific PR.

`admin.dashboard.read` is deprecated compatibility vocabulary only. Admin dashboard metrics use the DB-backed `roles.view:all` gate at the API/RPC boundary.
