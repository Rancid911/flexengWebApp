...
> Historical document. This is not current runtime truth. Active cleanup should be tied to current code, `docs/README.md`, and a concrete feature, bug, performance issue, or review finding.
...
- RBAC/RLS migration is tracked separately in `docs/decisions/historical/rbac-rls-implementation-plan.md`. Keep it split into small PRs: decision/inventory, additive schema, backfill, AppActor compatibility, teacher preview guard, permission pilot, RLS helpers, domain RLS rollouts, then service-role cleanup.
