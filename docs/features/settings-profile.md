...
Related code: `app/(workspace)/(shared-zone)/settings/`, `app/api/settings/profile/route.ts`, `features/settings/`, `lib/settings/profile.service.ts`, `lib/settings/profile.types.ts`, `lib/media/urls.ts`, `lib/media/service.ts`  
Related tests: `tests/unit/settings-profile-page.test.tsx`, `tests/unit/settings-profile-route.test.ts`, `tests/unit/settings-profile-service.test.ts`, `tests/unit/settings-form-state.test.tsx`, `tests/e2e/settings-routes.smoke.spec.ts`, `tests/e2e/settings.smoke.spec.ts`, `tests/unit/media-api-routes.test.ts`
...
- User edits profile fields, email, password or avatar and saves through `PATCH /api/settings/profile`.
...
| `/api/settings/profile` | PATCH | protected mutation | Requires `profile.update` scoped to `ownerUserId: actor.userId`; accepts form data for profile/email/password/avatar changes. |
...
- Supabase Auth user: canonical email, pending email and password updates.
...
- Password change verifies the current password before update.
...
- Supabase Auth for email/password updates.
- Supabase Database for profile/student fallback rows.
- Supabase Storage `avatars` bucket for avatar files.
...
- Settings profile service tests, including student birth-date fallback behavior.
...
- For profile save failures, inspect API response codes and `lib/settings/profile.service.ts`.
