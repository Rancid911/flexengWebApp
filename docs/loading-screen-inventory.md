...
| `/student-dashboard` | Student dashboard | `features/dashboard/server/student-dashboard-route.tsx` | `app/(workspace)/(student-zone)/student-dashboard/loading.tsx` | Student dashboard summary/progress cards and learning/payment sections | Implemented route-local screen-shaped fallback; browser validation still required | `updated` |
...
| `/words` | Redirect to `/words/my` | `app/(workspace)/(shared-zone)/words/page.tsx` | `app/(workspace)/loading.tsx` | Neutral workspace fallback | Redirect route; dedicated skeleton not useful | `no dedicated skeleton needed` |
| `/words/my` | Words overview/topics | `features/words/server/words-overview-routes.tsx` | `app/(workspace)/(shared-zone)/words/my/loading.tsx` | Vocabulary library filters, counters and word rows | Implemented route-local vocabulary library fallback; browser validation still required | `updated` |
