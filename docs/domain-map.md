...
| Admin | `app/(workspace)/(staff-zone)/admin`, `app/api/admin`, `lib/admin` | `watch` | Users, blog, tests, notifications, payments, word sets | Admin shell is expected to touch many domains; users, directory reads, course/module options, dashboard metrics, tests, word-card sets, blog, notifications and teacher dossier workflows now delegate to service/query/repository boundaries instead of raw route/page DB access. |
...
| API | `app/api/*`, `lib/server/http`, `lib/admin/http` | `watch` | Domain services, auth/request context | Generic and admin error wrappers exist; architecture checks now block raw Supabase/admin-client access in route handlers. Keep routes as transport layers that delegate to domain services. |
