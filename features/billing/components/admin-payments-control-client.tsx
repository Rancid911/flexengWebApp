      ...
      <section className="grid gap-5 xl:grid-cols-[1.5fr_0.9fr]">
                ...
                data.items.map((item) => (
                  <div key={item.student_id} className={cn("rounded-[1.4rem] border px-4 py-4", toneClass(item))}>
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <Link href={`/admin/students/${item.student_id}`} className={`inline-flex h-10 items-center px-4 text-sm transition ${workspacePrimaryButtonClassName}`}>
                          Оплата и списания
                        </Link>
                        <Button
                          type="button"
        ...
        <Card className="rounded-[2rem] border-[#dfe9fb] bg-white shadow-[0_14px_30px_rgba(15,23,42,0.04)]">
          <CardContent className="space-y-5 p-6">
            ...
            <Button type="button" onClick={() => void saveSettings()} disabled={settingsSaving} className={`h-11 px-4 ${workspacePrimaryButtonClassName}`}>
