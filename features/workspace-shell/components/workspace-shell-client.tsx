...
import type { DashboardMobileActionItem } from "@/features/workspace-shell/components/dashboard-mobile-actions-sheet";
import {
  useDashboardLogout,
...
const CRM_UNREAD_SUMMARY_INITIAL_DELAY_MS = 100;
...
function scheduleCrmUnreadSummaryUpdate(callback: () => void) {
  if (typeof window === "undefined") {
    return () => undefined;
  }

  if ("requestIdleCallback" in window) {
    const idleId = window.requestIdleCallback(callback, { timeout: CRM_UNREAD_SUMMARY_INITIAL_DELAY_MS });
    return () => window.cancelIdleCallback(idleId);
  }

  const timeoutId = globalThis.setTimeout(callback, CRM_UNREAD_SUMMARY_INITIAL_DELAY_MS);
...
function normalizeCrmBackgroundImageUrl(url: string | null | undefined) {
...
export function WorkspaceShellClient({
  ...
  const setCrmBackgroundImageUrl = useCallback((url: string | null) => {
    const nextUrl = normalizeCrmBackgroundImageUrl(url);
    startTransition(() => setCrmBackgroundImageOverride(nextUrl));
  }, []);
  const crmBackgroundContextValue = useMemo(
    () => ({
  ...
  useEffect(() => {
    ...
    cancelScheduledUpdate = scheduleCrmUnreadSummaryUpdate(updateUnreadSummary);
  ...
  useEffect(() => {
    const handleCrmBackgroundChange = (event: Event) => {
      const detail = event instanceof CustomEvent ? event.detail : null;
      setCrmBackgroundImageUrl(detail?.backgroundImageUrl ?? null);
    };
  ...
  return (
    <>
      ...
      >
        <CrmBackgroundContext.Provider value={crmBackgroundContextValue}>
          {children}
