...
export function getAdminDashboardHydrationSafeInitialState(initialMetrics?: AdminDashboardMetricsDto | null) {
  const metrics = initialMetrics ? normalizeMetrics(initialMetrics) : defaultMetrics;
  return {
    metrics,
    hasData: Boolean(initialMetrics),
    loading: !initialMetrics,
    error: ""
  };
}
...
export function useAdminDashboardMetrics(initialMetrics?: AdminDashboardMetricsDto | null) {
  const initialState = getAdminDashboardHydrationSafeInitialState(initialMetrics);
  ...
  useEffect(() => {
    if (initialMetrics) {
      const snapshot: AdminDashboardSnapshot = {
        cachedAt: Date.now(),
        metrics: normalizeMetrics(initialMetrics)
      };
  ...
  }, [initialMetrics, runMetricsRequest]);
