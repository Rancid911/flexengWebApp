...
import { AdminDrawer } from "@/shared/ui/workspace-drawer";
import { fetchJson } from "@/shared/client/api-client";
import { replaceLead, toLeadCard, useCrmLeadDrawerActions } from "@/features/crm/client/use-crm-lead-drawer-actions";
import { useCrmSettingsState } from "@/features/crm/client/use-crm-settings-state";
import { CrmBoardColumns, CrmLeadDrawerContent, CrmSettingsDrawerContent, CrmStats, CrmToolbar } from "@/features/crm/components/crm-board-components";
...
export function CrmBoardClient({ initialBoard, initialSettings = { background_image_url: null, updated_at: null } }: CrmBoardClientProps) {
  ...
  const setShellCrmBackgroundImageUrl = useCallback(
  ...
  const settingsState = useCrmSettingsState(initialSettings, {
    onBackgroundImageChange: setShellCrmBackgroundImageUrl
  });
  ...
  useEffect(() => {
    setShellCrmBackgroundImageUrl(initialSettings.background_image_url);
    window.dispatchEvent(
      new CustomEvent(CRM_BACKGROUND_IMAGE_EVENT, {
        detail: { backgroundImageUrl: initialSettings.background_image_url }
      })
    );
    return () => setShellCrmBackgroundImageUrl(null);
  }, [initialSettings.background_image_url, setShellCrmBackgroundImageUrl]);
