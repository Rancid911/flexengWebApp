...
import { useCurrentPageDirectoryFilter } from "@/features/students/client/use-current-page-directory-filter";
import { Card, CardContent } from "@/components/ui/card";
import { workspacePrimaryButtonClassName } from "@/shared/constants/workspace-button-tokens";
                ...
                <div key={teacher.teacherId} className="rounded-[1.35rem] border border-[#dfe9fb] bg-[#fbfdff] px-4 py-4">
                  <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center xl:shrink-0">
                      <Link href={`/admin/teachers/${teacher.teacherId}`} className={`inline-flex h-10 shrink-0 items-center justify-center px-4 text-sm transition ${workspacePrimaryButtonClassName}`}>
                        Открыть карточку
                      </Link>
                    </div>
                  </div>
