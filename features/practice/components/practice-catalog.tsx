...
import type { PracticeCatalogFilter, getPracticeActivityCatalog } from "@/lib/practice/queries";
import { StudentEmptyState, StudentPageHeader, StudentSubnav } from "@/shared/ui/student-page-primitives";

export const practiceCatalogFilters: Array<{ value: PracticeCatalogFilter; label: string }> = [
  { value: "all", label: "Все" },
...
type PracticeCatalogItems = Awaited<ReturnType<typeof getPracticeActivityCatalog>>;

export function PracticeCatalog({ filter, items }: { filter: PracticeCatalogFilter; items: PracticeCatalogItems }) {
  return (
    <div className="space-y-6 pb-8">
      ...
      <div className="flex flex-wrap gap-2">
        {practiceCatalogFilters.map((option) => (
          <Link
            key={option.value}
