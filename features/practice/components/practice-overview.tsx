...
import type { getPracticeRecommended, getPracticeTopics } from "@/lib/practice/queries";
import { StudentEmptyState, StudentPageHeader, StudentSubnav } from "@/shared/ui/student-page-primitives";

type RecommendedItem = Awaited<ReturnType<typeof getPracticeRecommended>>[number];
type TopicItem = Awaited<ReturnType<typeof getPracticeTopics>>[number];
...
export function PracticeOverview({
  ...
  doNowSlot: ReactNode;
  continueTopicSlot: ReactNode;
  weakSpotSlot: ReactNode;
}) {
  return (
...
export function PracticeDoNowCard({ item }: { item?: RecommendedItem | null }) {
  return (
...
export function PracticeContinueTopicCard({ topic }: { topic?: TopicItem | null }) {
  return (
...
export function PracticeWeakSpotCard({ item }: { item?: RecommendedItem | null }) {
  return (
    <Card className="rounded-[2rem] border-[#dde2e9] bg-white shadow-[0_12px_30px_rgba(15,23,42,0.05)]">
      <CardContent className="space-y-4 p-6">
        <div className="flex items-center justify-between gap-3">
