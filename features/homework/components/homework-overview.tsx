...
import { StudentPageHeader, StudentSubnav } from "@/shared/ui/student-page-primitives";
import { Card, CardContent } from "@/components/ui/card";
import { formatRuLongDateTime } from "@/lib/dates/format-ru-date";
import type { HomeworkListItem, HomeworkOverviewSummary } from "@/lib/homework/queries";
import { renderHomeworkList } from "@/features/homework/components/homework-list";
...
export function HomeworkOverview({ summary, items }: { summary: HomeworkOverviewSummary; items: HomeworkListItem[] }) {
  return (
...
export function HomeworkFilteredListPage({
  title,
  description,
  activeHref,
  items
  ...
  title: string;
  description: string;
  activeHref: "/homework/active" | "/homework/completed" | "/homework/overdue";
    ...
    <div className="space-y-6 pb-8">
      ...
      <StudentSubnav
        items={[
          ...
          { href: "/homework/active", label: "Активные", active: activeHref === "/homework/active" },
          { href: "/homework/completed", label: "Завершённые", active: activeHref === "/homework/completed" },
          { href: "/homework/overdue", label: "Просроченные", active: activeHref === "/homework/overdue" }
        ]}
      />
      {renderHomeworkList(items)}
    </div>
